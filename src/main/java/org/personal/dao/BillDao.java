...
public class BillDao {
	...
	public Bill selectBill(int id) throws ClassNotFoundException {
		Bill bill = null;
		try {
			final String QUERY = "select username, month, total_amount, is_paid from bill where id =?";
			connection = ConnectionFactory.getConnection();
			preparedStatement = connection.prepareStatement(QUERY);
			...
			while (rs.next()) {
				...
				bill = new Bill(id, username, month, totalAmount, isPaid);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public boolean deleteBill(int id) throws SQLException, ClassNotFoundException {
		boolean rowDeleted;
	...
	public boolean updateBill(Bill bill) throws SQLException, ClassNotFoundException {
		boolean rowUpdated;
		final String QUERY = "UPDATE bill set month= ?, total_amount = ?, is_paid = ? WHERE id = ? ";
		connection = ConnectionFactory.getConnection();
		PreparedStatement preparedStatement = connection.prepareStatement(QUERY);     
